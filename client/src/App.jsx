...
function App() {
  ...
  const handleSubmit=()=>{
    console.log(postData)
     axios.post("http://localhost:4000/api/post",{task:postData}).then((res) => {
    ...
    <>
      ...
      <h1 className="text-[40px] font-bold mb-5 ">ToDo App</h1>
        <div>
          ...
          />
          <br />
          <button onClick={handleSubmit} className="border border-black w-[70%] bg-purple-400 p-2 rounded-lg hover:bg-purple-600">
            Submit
          </button>
        </div>
        ...
        <div className=" mt-11  flex flex-col justify-items-center items-center">
          {data.map((res) => {
            ...
            <TaskC task={res.task}  key={res._id} id={res._id}/>
            );
          })}
        </div>
      </div>
